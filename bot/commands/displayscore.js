			...
			try {
				...
				if (statusCode != 200) {
					...
					getPlayerName(ev, ranking.stat._id, function(err, playerName) {
						...
						var response = '```md\n';  
						response += '< ' + playerName + ' > is on ranking <' + player.ranking.score + '> with a score of <' + Math.round(player.score) + '> and a win percentage of <' + (player.wins / player.games * 100).toFixed(2) + '%> out of <' + player.rankedGames + '> games. More info:\n\n' +   
