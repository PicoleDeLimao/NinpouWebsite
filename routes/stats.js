...
router.get('/reset_date', function(req, res) {
	Game.find({ }, function(err, games) {
		for (var i = 0; i < games.length; i++) {
			games[i].createdAt = dateFromObjectId(games[i]._id.toString());
			games[i].save(function() { 
					...
					if (index >= game.slots.length || index >= 9) {
						...
						getPlayerAlias(game.slots[index].username.toLowerCase(), function(err, username) {
							...
							Stat.findOne({ username: game.slots[index].username.toLowerCase() }, function(err, stat) {
								...
								var newPoints = stat.kills * 10 + stat.assists * 2 - stat.deaths * 5;
								if (Math.floor(oldPoints) != Math.floor(newPoints)) {
									changes.push({ alias: stat.username, oldPoints: oldPoints, newPoints: newPoints });
								}
